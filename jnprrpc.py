...
class ncfrpc:
    ...
    def connectdev(self):
        jp = nc.connect(host=self.host, username=self.uname, password=self.passwd, port=self.nport, hostkey_verify=False, device_params={'name':'junos'})
        return jp


    def get_interface(self):
        filter = '<get-interface-information><terse/></get-interface-information>'
        try:            # with nc.connect(host=self.host, username=self.uname, password=self.passwd, port=self.nport, hostkey_verify=False, device_params={'name':'junos'}) as jp:
            jp = self.connectdev()
            data = jp.rpc(filter)
